...
constexpr uint8_t CONTEXT_TAG {0U};
...
{
protected:
    void SetUp() override { _context = std::make_unique<LogContext>(CONTEXT_TAG, "test", _sink, _timeProvider); }

    Logger makeLogger() { return Logger {*_context}; }

    CapturingLogSink _sink;
    FixedTimeProvider _timeProvider;
...
{
    Logger logger = makeLogger();
    _context->setLogFilterLevel(LogLevel::ERROR);
